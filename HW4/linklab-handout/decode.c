...
void decode(instruction_t *ins, code_t *code_ptr, Elf64_Addr code_addr)
{
  ...
  if ((code_ptr[0] == 0x48)
    ...
    ins->length += 7;
    ins->mov_addr_to_reg.addr = code_addr + 7 + rel_addr;
    return;
  }
  ...
  if ((code_ptr[0] == 0xFF)
    ...
    ins->length += 6;
    ins->jmp_to_addr.addr = code_addr + 6 + rel_addr;
    return;
  }
  ...
  if ((code_ptr[0] >= 0x70)
    ...
    ins->length += 2;
    ins->maybe_jmp_to_addr.addr = code_addr + 2 + rel_addr;
    return;
  }
...
void replace_with_crash(code_t *code_ptr, instruction_t *ins)
{
  ...
  if (ins->length < 2) {
    fprintf(stderr, "cannot crash withing too-small length %d\n",
	    ins->length);
    exit(1);
  }
