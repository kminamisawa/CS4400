...
(module+ main
  (command-line
   ...
   (for ([p (in-list predictions)])
     (try-call so-file (prediction-function p) (prediction-arg p) (prediction-result p)))
   (when (zero? status)
     (printf "Passed\n"))
   (exit status)))
...
(define (try-call so-file function arg result)
  (define so-dest "dest.so")
  (unless (system* "./redact" so-file so-dest)
    (fprintf (current-error-port) "redact failed\n")
    (exit 1))
  ...
  (cond
    ...
    [else
     ...
     (unless (equal? result got)
       (bad! (format "expected ~a(~a) => ~a, got ~a" function arg result got)))]))
